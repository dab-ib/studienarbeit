...
export const cameraStoreMutations = {
  ...
  remove: (id: number) => {
    const removeIndex = cameraStoreState.camerasArray
      .map((cam) => {
        return cam.id;
      })
      .indexOf(id);
  ...
  getIndex: (id: number) => {
    return cameraStoreState.camerasArray.indexOf(
      cameraStoreState.camerasArray.find((e) => e.id == id) as Camera
    );
  },
  getList: async () => {
